  ...
  return (
    <div className="relative grid grid-cols-4 grid-rows-5 gap-px w-28 aspect-[4/5] bg-neutral-950 rounded-lg p-1 mx-auto my-3">
      ...
      <div
        ...
        style={{ width: "calc((100% - 0.5rem) / 2)", height: "calc((100% - 0.5rem) * 2 / 5)" }}
      />
      {/* Beige 1x1 */}
      <div className="rounded-sm bg-[#e8d8c9]" style={{ gridArea: "1 / 1 / 2 / 2" }} />
      {/* Bleu 2x1 horizontal */}
      <div className="rounded-sm bg-[#4b607f]" style={{ gridArea: "1 / 2 / 2 / 4" }} />
      {/* Beige 1x1 */}
      <div className="rounded-sm bg-[#e8d8c9]" style={{ gridArea: "1 / 4 / 2 / 5" }} />
      {/* Rouge 2x2 */}
      <div className="rounded-sm bg-[#f3701e]" style={{ gridArea: "2 / 2 / 4 / 4" }} />
      {/* Beige 1x1 */}
      <div className="rounded-sm bg-[#e8d8c9]" style={{ gridArea: "2 / 4 / 3 / 5" }} />
      {/* Bleu 1x2 vertical */}
      <div className="rounded-sm bg-[#4b607f]" style={{ gridArea: "2 / 1 / 4 / 2" }} />
      {/* Bleu 2x1 horizontal */}
      <div className="rounded-sm bg-[#4b607f]" style={{ gridArea: "4 / 1 / 5 / 3" }} />
      {/* Beige 1x1 */}
      <div className="rounded-sm bg-[#e8d8c9]" style={{ gridArea: "4 / 3 / 5 / 4" }} />
      {/* Beige 1x1 */}
      <div className="rounded-sm bg-[#e8d8c9]" style={{ gridArea: "3 / 4 / 4 / 5" }} />
      {/* Beige 1x1 */}
      <div className="rounded-sm bg-[#e8d8c9]" style={{ gridArea: "5 / 1 / 6 / 2" }} />
      {/* Beige 1x1 */}
      <div className="rounded-sm bg-[#e8d8c9]" style={{ gridArea: "4 / 4 / 5 / 5" }} />
      {/* Beige 1x1 */}
      <div className="rounded-sm bg-[#e8d8c9]" style={{ gridArea: "5 / 2 / 6 / 3" }} />
      {/* Vide: row 5 cols 3-4 */}
    </div>
  );
}
...
function InlinePiece({ color, size = "1x1" }: { color: string; size?: "1x1" | "2x1" | "2x2" }) {
  const sizeClass = size === "2x2" ? "w-5 h-5" : size === "2x1" ? "w-7 h-3.5" : "w-3.5 h-3.5";
  return (
    <span
      ...
      <div className="p-4 space-y-5 text-sm leading-relaxed">
        ...
        <section>
          ...
          <div className="flex items-start gap-3 mb-2">
            <ArrowsOutCardinalIcon weight="bold" className="w-6 h-6 text-[#f3701e] shrink-0 mt-0.5" />
            <p>
              Les pièces se déplacent horizontalement ou verticalement
              et ne peuvent pas se chevaucher.
            </p>
          </div>
          <div className="space-y-1.5 text-xs text-[#e8d8c9]/70">
            <p>
              <InlinePiece color="#f3701e" size="2x2" /> Pièce rouge (2x2) — c&apos;est celle qu&apos;il faut amener à la sortie
            </p>
            <p>
              <InlinePiece color="#4b607f" size="2x1" /> Pièces bleues (2x1) — elles bloquent le passage
            </p>
            <p>
              <InlinePiece color="#e8d8c9" /> Pièces beiges (1x1) — elles bloquent aussi
            </p>
          </div>
        </section>
        ...
        <section>
          ...
          <div className="space-y-3">
            <MiniButton
              color="#4b607f"
              icon={<CaretLeftIcon weight="fill" className="w-3 h-3 text-[#e8d8c9]" />}
              label="Niveau précédent"
            />
            <MiniButton
              color="#4b607f"
              icon={<CaretRightIcon weight="fill" className="w-3 h-3 text-[#e8d8c9]" />}
            ...
            <MiniButton
              color="#f3701e"
              icon={<CircleIcon weight="bold" className="w-3 h-3 text-[#e8d8c9]" />}
        ...
        <section>
          ...
          <ul className="space-y-1.5 text-xs text-[#e8d8c9]/70 list-none">
            ...
            <li>Le chronomètre se lance et les LEDs s&apos;éteignent une à une</li>
            <li>Le puzzle doit être résolu avant que le temps soit écoulé</li>
            <li>
              Le score va de{" "}
              <span className="text-[#f3701e] font-bold">S</span> (rapide) à{" "}
              <span className="text-[#f3701e] font-bold">F</span> (lent)
            </li>
          </ul>
        </section>
